...
public class DepthSourceView : MonoBehaviour
{
    ...
    private double GetAvg(CameraSpacePoint[] cameraData, int x, int y, int width, int height)
    {
        double sum = 0.0;
        int count = 0;
        ...
        for (int y1 = y; y1 < y + 4; y1++)
        {
            for (int x1 = x; x1 < x + 4; x1++)
            {
                ...
                var val = cameraData[fullIndex].Z;


                if (val != 0 && !double.IsInfinity(val))
                {
                    count++;
                    sum += val;
                }
